...
.service('DataService', function(LocalStorageService) {
	return {
		...
		*/
		...
		localMobileUser_currentCheckpoint: function(cb) {
		 ...
		 * 		the following parameters cb(err, currentCheckpoint)
		 */
		remoteMobileUser_currentCheckpoint: function(cb) {
		 ...
		 * 		the following parameters cb(err, currentCheckpoint)
		 */
		resetLocalModels: function(cb) {
			this.sendMessage("resetlocalmodels", null, null, cb);
		},

    /**
		 * @method localGolfReport_find
		 *
		 * @description Function gets all local Golf Reports specified by the included filter and passes them to the provided
		 * callback function.
		 *
		 * @param {Object} filter Filter object to specify which Golf Reports to return.  Ex.
		 * 		{where: {field: "value"}}
		 * @param {function} cb Callback function to be executed after the remote call is executed.  Will be passed
		 * 		the following parameters cb(err, res)
		 * @return void
		 * @throws None
		 */
    localGolfReport_find: function(filter, cb) {
      this.sendMessage("localgolfreport.find", null, filter, cb);
    },
